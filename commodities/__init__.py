...
Commodity codes are 10-digit codes that refer to specific products. Traders must put
these codes on their declaration forms when importing or exporting goods.

Nomenclature, also known as goods classification, goods nomenclature or commodity code
classification, is the name for the full list of products in the UK Tariff.
...
1. Do we have a correct commodity tree after the change?
   ...
   - the question applies for the changing commodity as well as all commodities in its
...
2. Have we dealt with any side effects on any related taric records
   ...
   - affected records may be related to the changing commodity itself or to any
     commodity in the surrounding hierarchy pre- and post-change (e.g. business rule
     NIG30 might mean we need to terminate a measure)

   - side effects are incidental violations of business rules resulting from the changes
...
In order to handle well commodity tree chanegs in the context of the above two main
considerations, we need to adopt a new approach that:
...
The new approach in TP-803 satisfies the above criteria by "bootstrapping" any pending
...
record changes that need to be applied alongside the commodity changes.

The following constructs make the bootstrapping approach possible:

1. Commodity wrapper

   - the primary benefit of the wrapper is the ability to "fake" changes to the wrapped
     underlying record for the purposes of previewing the effects and side effects of
     the change.

   - one example is "masking" relations fields to avoid complications of dealing with
     related meta records (e.g. indents in commodities).
...
3. CommodityTreeSnapshot
   ...
   - a snapshot is a collection of commodities that are in effect as of a given moment,
     and constitute the tree hierarchy as of that moment
   ...
   - a snapshot has a tree hierarchy with parent, children and sibling relations as well
     as traversal-style relations such as ancestors and descendants
...
4. SnapshotDiff
   ...
   - a diff is evaluated in terms of members of a single relation for a single commodity
     ...
     - e.g. what is the difference in the siblings of commodity x between snapshot a and
       snapshot b?
   ...
   - there are two motivations for using a snapshot diff:
     ...
     - compare the state of the commodity tree at two different points in time (even
       outside the context of a commodity change)
...
5. CommodityChange
   ...
   - this construct serves as a "watchdog" for pending changes:
     ...
     - it evaluates the "sanity" of a requested change (e.g. if someone requests an
       update to a non-existing commodity)

     - it evaluates and determines remedies for any side effects incidental to the
       pending change
...
7. CommodityTreeLoader
   ...
   - responsible for loading any subset of the tariff db commodity tree into a
     CommodityCollection (up to a chapter level)
...
With the above in mind, the intended workflow that TP-803 envisions (parts of which are
implemented elsewhere) is the following:
...
3. The pending commodity changes are wrapped in CommodityChange instances
   ...
   - this involves collection updates, taking "before" and "after" snapshots, plus
     probing for any related records that might be affected by the change
...
5. At this point we have everything we need in order to be able to write changes to the
   tariff db that have the intended effect on the tree hierarchy and remedy any side
   effects on any related records caught up in the change
...
All of the above can be viewed simply as a holding bay slash decision engine; no action
...
of import handlers and is implemented as a separate feature (see TP-931).
"""
