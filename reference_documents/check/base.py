...
class BaseCheck(abc.ABC):
    """
    Abstract Base class, only used to provide a common parent and behaviour to
    child classes that will be used as checks for all reference document
    objects.

    This class contains shared functionality that is used by child classes
    throughout.
    """
    ...
    def run_check(self) -> (AlignmentReportCheckStatus, str):
        ...
        This method on inheriting classes will be called to execute the check
...
class BaseQuotaDefinitionCheck(BaseCheck, abc.ABC):
    """
    Abstract Base class, only used to provide a common parent and behaviour to
    child classes that will be used as checks for quota definitions.
    ...
    def __init__(self, ref_quota_definition: RefQuotaDefinition):
        ...
        self.ref_order_number = self.ref_quota_definition.ref_order_number
    ...
    def commodity_code(self):
        """
        Finds the latest approved version of a commodity code in TAP of a given
        preferential quota.
        ...
        goods = GoodsNomenclature.objects.latest_approved().filter(
            Q(
                valid_between__contains=TaricDateRange(
                    self.ref_quota_definition.valid_between.lower,
                    self.ref_quota_definition.valid_between.upper,
                ),
            )
            | Q(
    ...
    def quota_definition(self):
        """
        Searches for the quota definition period in TAP of a given preferential
        quota.
        ...
        if len(measures) == 1:
            ...
            for index, measure in enumerate(measures):
                ...
                if measure1_end_date + timedelta(days=1) != measure2_start_date:
                    ...
                    measure_span_periods[-1] = TaricDateRange(
                        measure_span_periods[-1].lower,
    ...
    def get_tap_association(self) -> Optional[QuotaAssociation]:
        """
        Queries and returns the TAP association for a given preferential quota
        if available.
    ...
    def tap_association_exists(self):
        """
        Queries the TAP association for a given preferential quota and returns
        boolean representing the presence of that association.
    ...
    def check_coefficient(self):
        """
        Checks the TAP coefficient against the reference document expected
        coefficient and compares.
    ...
    def duty_rate_matches(self):
        """
        Checks the TAP duty rate against the reference document expected duty
        rate and compares.
...
class BaseOrderNumberCheck(BaseCheck, abc.ABC):
    """
    Abstract Base class, only used to provide a common parent and behaviour to
    child classes that will be used as checks for order numbers.
...
class BaseQuotaSuspensionCheck(BaseCheck, abc.ABC):
    """
    Abstract Base class, only used to provide a common parent and behaviour to
    child classes that will be used as checks for quota suspensions.
    ...
    def tap_quota_definition(self):
        ...
        """
        order_number = (
            self.ref_quota_suspension.ref_quota_definition.ref_order_number.order_number
        )
...
class BaseRateCheck(BaseCheck, abc.ABC):
    """
    Abstract Base class, only used to provide a common parent and behaviour to
    child classes that will be used as checks for preferential rates.
    ...
    def get_snapshot(self, comm_code=None) -> Optional[CommodityTreeSnapshot]:
        """
        Gets a commodity tree snapshot from the provided comm code item id, if
        an item_id is available.
    ...
    def tap_geo_area_description(self) -> Optional[str]:
        """
        Finds the latest approved version of a geographical area description in
        TAP for the related geographical area.
    ...
    def ref_doc_version_eif_date(self):
        """
        Returns the EIF (Entry into force) date for a reference document
        version.

        Returns:
            date or None: if available, the EIF date else none
        """
        eif_date = self.ref_rate.reference_document_version.entry_into_force_date
    ...
    def tap_related_measures(self, comm_code_item_id: str = None):
        """
        Queries and returns measures related to a given comm code item id
        limited to measures that have a valid between that overlaps the required
        period defined by the reference document rate.
