...
from reference_documents.models import CSVUpload, ReferenceDocumentVersion, ReferenceDocumentVersionStatus, ReferenceDocument, RefRate, RefOrderNumber, RefQuotaDefinition, ReferenceDocumentCsvUploadStatus
...
class ReferenceDocumentCSVImporter:
    ...
    def run(self):
        ...
        if not self.csv_upload.preferential_rates_csv_data and not self.csv_upload.order_number_csv_data and not self.csv_upload.quota_definition_csv_data:
    ...
    def find_reference_document_version(self, reference_document_version: float, reference_document: ReferenceDocument, status=None):
        """
    ...
    def verify_area_id_exists(area_id):
        if not GeographicalArea.objects.latest_approved().filter(area_id=area_id).exists():
    ...
    def verify_comm_code(self, comm_code):
        if not bool(re.match('^[0123456789]+$', comm_code)):
            raise ValueError(f'{comm_code} is not a valid comm code, it can only contain numbers')

        if len(comm_code) != 10:
            raise ValueError(f'{comm_code} is not a valid comm code, it should be 10 characters long')


    def import_preferential_rates_csv_data(self):
        logger.info(f" -- IMPORTING PREFERENTIAL RATES", )
        data = self.get_dictionary_from_csv_data(self.csv_upload.preferential_rates_csv_data)
        ...
        for header in expected_headers:
            if header not in data[0].keys():
                raise ValueError(f'CSV data for preferential rates missing header {header}')
        ...
        for row in data:
            ...
            start_date = datetime(*[int(x) for x in row['validity_start'].split('-')]).date()
            end_date = row['validity_end']

            if end_date == '':
                end_date = None
            else:
                end_date = datetime(*[int(x) for x in row['validity_end'].split('-')]).date()

            if matching_row.exists():
                raise Exception(f'Preferential Rate already exists, details : {row}, matched on commodity_code and start_date.')
            else:
    ...
    def get_or_create_reference_document_version(self, row):
        ...
        if reference_document_version:
            if reference_document_version.status != ReferenceDocumentVersionStatus.EDITING:
                raise Exception(f'Reference document version {reference_document_version.reference_document.area_id}:{reference_document_version.version} has status {reference_document_version.status} and can not be altered.')
        else:
    ...
    def process_order_number(self, row):
        ...
        if matching_row.exists():
            raise Exception(f'Order Number already exists, details : {row}, matched on order number and start_date.')
        else:
            ...
            if parent_order_number == '':
                ...
                parent_order_number_query = reference_document_version.ref_order_numbers.filter(order_number=parent_order_number)
                if parent_order_number_query.exists():
                    parent_order_number = parent_order_number_query.first()
                else:
                    raise Exception(f'Parent Order Number {parent_order_number} does not exist.')
    ...
    def import_quota_definition_csv_data(self):
        data = self.get_dictionary_from_csv_data(self.csv_upload.quota_definition_csv_data)
        ...
        for header in expected_headers:
            if header not in data[0].keys():
                raise ValueError(f'CSV data for quota definitions missing header {header}')
        ...
        for row in data:
            ...
            order_number = reference_document_version.ref_order_numbers.filter(order_number=row['order_number'])
            ...
            if matching_row.exists():
                raise Exception(f'Quota Definition already exists, details : {row}, matched on commodity_code, order number and start_date.')
            else:
