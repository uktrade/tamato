...
class TrackedModelQuerySet(PolymorphicQuerySet):
    ...
    def approved(self):
        ...
        # select * from trackedmodel m
        # inner join transaction tx on m.transaction_id = tx.id
        # inner join workbaskettransaction wt on wt.transaction_id = tx.id
        # inner join workbasket w on wt.workbasket_id = w.id
        # where w.status in approved_statuses and w.approver is not null

        approved_workbaskets = WorkBasket.objects.filter(
            status__in=WorkflowStatus.approved_statuses(),
            approver__isnull=False,
        )

        return self.filter(
            transaction__workbaskets__in=approved_workbaskets,
        )
