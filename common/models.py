...
class TrackedModelQuerySet(PolymorphicQuerySet):
    def latest(self) -> QuerySet:
        """
        Get all the latest versions of the model being queried.

        Latest is defined as the last row relating to the history of an object.
        ...
        If done from the TrackedModel this will return the latest objects for all tracked
        models.
        """
        return self.filter(predecessor__isnull=True)
    ...
    def since_transaction(self, transaction_id: int) -> QuerySet:
        ...
        """
        return self.filter(workbasket__transaction_id__gt=transaction_id)
    ...
    def get_current_version(self, **kwargs):
        ...
        """
        return self.current().get_version(**kwargs)
...
class TrackedModelManager(PolymorphicManager):
    queryset_class = TrackedModelQuerySet


class PolymorphicMPTreeManager(TrackedModelManager):
    queryset_class = PolymorphicMPTreeQuerySet
