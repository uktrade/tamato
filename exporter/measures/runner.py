...
class measureExport:
    ...
    def run(self):
        ...
        measures_now = Measure.objects.latest_approved().filter(
            filter_query,
            sid__gte=20000000,
        )
        ...
        with open(self.target_file.name, "wt") as file:
            ...
            for measure in measures_now:
                ...
                measure_data = [
                    ...
                    "commodity__description",  #
                    measure.sid,  # sid
                    # 'measure__type__id',  # FK measure.measure_type measure_types."sid"
                    type_sid,
                    # 'measure__type__description',  # FK
                    MeasureType.objects.latest_approved().get(sid=type_sid).description,
