...
class QuotaExport:
    ...
    def run(self):
        ...
        quotas = ReportQuotaDefinition.objects.filter(
            sid__gte=20000,
    ...
    def get_geographical_areas_and_exclusions(quota):
        ...
        for origin in ReportQuotaOrderNumberOrigin.objects.filter(
            ...
            geographical_areas.append(
                origin.geographical_area.descriptions.all()
                .order_by("trackedmodel_ptr")
                .last()
                .description,
            )
            for exclusion in origin.reportquotaordernumberoriginexclusion_set.all():
                geographical_area_exclusions.append(
                    f"{exclusion.excluded_geographical_area.descriptions.all().order_by('trackedmodel_ptr').last().description} "
                    f"excluded from {origin.geographical_area.descriptions.all().order_by('trackedmodel_ptr').last().description}",
                )
    ...
    def get_api_query_date(quota):
        ...
        if quota.valid_between.lower <= date.today():
            ...
            tap_measures = quota.order_number.reportmeasure_set.all().filter(
