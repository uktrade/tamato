...
class WTOMeasureRow:
    def __init__(self, row: List[Cell]) -> None:
        ...
        self.quota_number = str(row[col("C")].value)
            ...
            with EnvelopeSerializer(
                ...
                for membership in GeographicalMembership.objects.filter(
                    ...
                    if (
                        ...
                        .filter(geo_group=erga_omnes, member=membership.member)
                        .exists()
                    ):
                        logger.debug(
                            "Adding %s to %s",
                ...
                quota_importer.import_sheets(
                    (r for r in quota_rows if r.source == QuotaSource.WTO),
                    iter([None]),
                )
