...
from exporter.report_exporter.tasks import export_and_upload_measure_csv
...
class Command(BaseCommand):
    help = (
        ...
        "measure_export_<yyyymmdd>.csv. Care should be taken to ensure that "
    ...
    def handle(self, *args: Any, **options: Any) -> Optional[str]:
        logger.info(f"Triggering measure export to CSV")

        local_path = options["DIRECTORY_PATH"]
        if options["asynchronous"]:
            export_and_upload_measure_csv.delay(local_path)
        else:
            export_and_upload_measure_csv(local_path)
