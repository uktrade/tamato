...
class FilterShowMore {
    ...
    init() {
        ...
        if (this.hasBeenRevealed()) {
            ...
            this.button.addEventListener('click', () => that.revealChoices());
        }
    }
    hasBeenRevealed() {
    ...
    revealChoices() {
        ...
        let choices = Array.from(this.module.getElementsByClassName('app-js-hidden'));
    ...
    checkboxSets.forEach((checkboxSet) => {
        if (checkboxSet.children.length > 10) {
            Array.from(checkboxSet.children).forEach((checkbox, index) => {
                if (index > 9) {
                    checkbox.classList.add('app-js-hidden')
                }
            })
