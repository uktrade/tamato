...
class TestBaseQuotaDefinitionCheck:
    ...
    def test_init(self):
        ...
        assert (
            ...
            == ref_quota_definition.ref_order_number.reference_document_version.reference_document
        )
    ...
    def test_commodity_code_match(self):
        ...
        pref_quota = factories.RefQuotaDefinitionFactory.create(
            ...
            valid_between=TaricDateRange(
                comm_code_start_date + timedelta(days=200),
                comm_code_start_date + timedelta(days=300),
            ),
        )
        target = self.Target(pref_quota)
    ...
    def test_quota_definition_match(self):
        ...
        target = self.Target(ref_quota_definition)
        assert (
            target.ref_quota_definition.ref_order_number.order_number
            == tap_quota_definition.order_number.order_number
        )
        assert target.ref_quota_definition.volume == tap_quota_definition.volume
        assert (
            target.ref_quota_definition.valid_between
            == tap_quota_definition.valid_between
        )
    ...
    def test_measures_match_single_measure(self):
        ...
        tap_measure = MeasureFactory.create(
            ...
            goods_nomenclature__valid_between=TaricDateRange(
                date.today() + timedelta(days=-100),
            ),
    ...
    @pytest.mark.parametrize(
        "test_name, measure_date_ranges,expected_measure_count",
        [
            (
    ...
    def test_measures_return_values(
        self,
        test_name,
        measure_date_ranges,
        ...
        quota_definition_validity = TaricDateRange(
            date.today() + timedelta(days=-10),
            date.today() + timedelta(days=10),
        )
    ...
    def test_get_tap_association_exists_fails_order_number_does_not_have_main_order_number(
        self,
    ):
        ref_sub_quota_definition = factories.RefQuotaDefinitionFactory.create()
        target = self.Target(ref_sub_quota_definition)
    ...
    @pytest.mark.parametrize(
        "test_name, measure_date_ranges,expected_result",
        [
            (
    ...
    def test_measures_cover_quota_definition_validity_period(
        self,
        test_name,
        measure_date_ranges,
        expected_result,
        ...
        assert (
            target.measures_cover_quota_definition_validity_period() == expected_result
        )
...
class TestBaseOrderNumberCheck:
    ...
    def test_init(self, ref_order_number=None):
        ...
        assert (
            "Can't instantiate abstract class BaseOrderNumberCheck without an implementation for abstract method 'run_check'"
            in str(e.value)
        )
    ...
    def test_tap_order_number_calls_super_if_provided(self):
        ...
        assert (
            target.tap_order_number(ref_order_number.order_number) == tap_order_number
        )
...
class TestBaseQuotaSuspensionCheck:
    ...
    def test_tap_order_number_matches(self):
        ...
        target = self.Target(ref_quota_suspension)
        assert (
            target.tap_order_number()
            == tap_quota_suspension.quota_definition.order_number
        )
    ...
    def test_tap_order_number_matches_when_order_number_provided(self):
        ...
        assert (
            target.tap_order_number(
                tap_quota_suspension.quota_definition.order_number.order_number,
            )
            == tap_quota_suspension.quota_definition.order_number
        )
...
class TestBaseRateCheck:
    ...
    @pytest.mark.parametrize(
        "test_name, comm_code_structure, expected_result",
        [
            (
    ...
    def test_tap_recursive_comm_code_check(
        self,
        test_name,
        comm_code_structure,
        expected_result,
