...
class TestQuotaExport:
    ...
    def test_get_geographical_areas_and_exclusions(self):
        ...
        exclusion = factories.QuotaOrderNumberOriginExclusionFactory(origin=quota.order_number.quotaordernumberorigin_set.first())

        target = self.get_target()
        geo_areas, geo_area_exclusions = target.get_geographical_areas_and_exclusions(quota)
        for origin in quota.order_number.quotaordernumberorigin_set.all():
            assert origin.geographical_area.descriptions.all().last().description in geo_areas
        assert exclusion.excluded_geographical_area.descriptions.all().last().description in geo_area_exclusions
    ...
    def test_get_monetary_unit_populated(self):
        ...
        target = self.get_target()
        assert target.get_monetary_unit(quota) == f'{quota.monetary_unit.description} ({quota.monetary_unit.code})'
    ...
    def test_get_measurement_unit_populated(self):
        quota = factories.QuotaDefinitionFactory(
            ...
            monetary_unit=None,
            measurement_unit=factories.MeasurementUnitFactory(code='AAA', description='BBB')
        )
