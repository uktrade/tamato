...
function QuotaOriginExclusionFormset({ origin, originIndex, options, errors, addEmptyExclusion, removeExclusion }) {

    return (
        <div className="govuk-inset-text" aria-live="polite">
            {origin.exclusions.map((exclusion, i) =>
                <QuotaOriginExclusionForm exclusion={exclusion} origin={origin} key={exclusion.id} options={options} index={i} originIndex={originIndex} removeExclusion={removeExclusion} errors={errors} />
            )}
