...
class TestNewQuotaEventParser:
    ...
    target_parser_class = NewQuotaUnblockingEventParser
    ...
    def test_it_handles_population_from_expected_data_structure_quota_balance_event(
        ...
        assert target.unblocking_date == "2021-07-04"

        assert target.data == '{"unblocking.date": "2021-07-04"}'

    def test_import(self, superuser):
        importer = preload_import(
            "quota_unblocking_event_CREATE.xml",
            __file__,
        )
        ...
        assert target.occurrence_timestamp == datetime(2020, 9, 25, 14, 58, 58)
        assert target.unblocking_date == "2021-01-01"

        assert target.data == '{"unblocking.date": "2021-01-01"}'
    ...
    def test_import_update(self, superuser):
        preload_import("quota_unblocking_event_CREATE.xml", __file__, True)
        importer = preload_import("quota_unblocking_event_UPDATE.xml", __file__)
    ...
    def test_import_delete(self, superuser):
        preload_import("quota_unblocking_event_CREATE.xml", __file__, True)
        importer = preload_import("quota_unblocking_event_DELETE.xml", __file__)
