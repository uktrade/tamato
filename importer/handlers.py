...
class BaseHandlerMeta(type):
    ...
    checks for their existence and type. If they aren't properly define then an error is raised at compile
    time (i.e. on import).

    The second requirement is that handlers need to be attached to the nursery. This os  so that the nursery
...
class BaseHandler(metaclass=BaseHandlerMeta):
    ...
    @classmethod
    def register_dependant(cls, dependant: BaseHandler):
        """
