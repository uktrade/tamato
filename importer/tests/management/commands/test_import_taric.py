...
class TestImportTaricCommand(TestCommandBase):
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                "Error: the following arguments are required: taric3_file, name, author",
            ),
            (
                ["foo"],
                pytest.raises(CommandError),
                "Error: the following arguments are required: name, author",
            ),
            (
                ["foo", "bar", "author"],
    ...
    def test_help(self, capsys):
        ...
        assert "taric3_file name author" in out
