...
The measures export system will query the TAP database for published measure data and store in a CSV
file.
...
from exporter.measures import runner
from exporter.measures import tasks


def make_export(measures_csv_named_temp_file: NamedTemporaryFile):

    measure_csv_exporter = runner.measureExport(measures_csv_named_temp_file)
    measure_csv_exporter.run()
    # do stuff
    # measures_csv_named_temp_file.close()
