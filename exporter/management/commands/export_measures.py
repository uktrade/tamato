...
from exporter.measures.tasks import export_and_upload_measures_csv
...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> Optional[str]:
        logger.info(f"Triggering measures export to CSV")

        local_path = options["DIRECTORY_PATH"]
        if options["asynchronous"]:
            export_and_upload_measures_csv.delay(local_path)
        else:
            export_and_upload_measures_csv(local_path)
