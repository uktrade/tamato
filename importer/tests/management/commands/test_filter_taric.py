...
class TestFilterTaricCommand:
    ...
    def test_dry_run(self):
        out = self.call_command_test(None, None, False, f"{self.example_goods_taric_file_location()}", "test_name", 'some_values')
    ...
    def test_dry_run_error_no_args(self):
        ...
        assert 'Error: the following arguments are required: file, name, values' in str(ex)
    ...
    def test_help(self):
        ...
        assert 'file name [values [values ...]]\n\n' \
        ...
        assert 'values                String value that the XML tag should have to be' in out
