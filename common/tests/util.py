...
def get_table_content(table, col_start=0):
    """
    :param table:  BeautifulSoup object
    :param col_start:  First column, set to ignore column.
    :return: Generate dicts based the table headers.

    Caveats:
    - Only one table header row is considered.
    - Does not take handle tables using rowspan or colspan.
    """
    headers = [th.getText().strip() for th in table.findAll("th")][col_start:]
    for row in table.findAll("tr"):
        if not row:
            break

        cols = list(row.findAll("td"))[col_start:]
        if not cols:
            continue

        data = {headers[i]: col.getText().strip() for i, col in enumerate(cols)}
        yield data
...
def assert_table_displays_models(table, tracked_models):
    ...
    table_data = list(get_table_content(table, col_start=1))

    for obj in tracked_models:
        # The for loop consumes tracked_models, putting them into a list
        # can aid debugging.
        expected = {
            "ID": f'{obj.get_described_object() if hasattr(obj, "get_described_object") else obj.structure_code}',
            "Type": f"{obj._meta.verbose_name.title()}",
            "Description": obj.structure_description or "-",
            "Activity date": f"{obj.transaction.updated_at:%d %b %Y}",
        }

        assert (
            expected in table_data
        ), f"Could not find {expected} in table {table_data}"
