...
class WorkBasket(TimestampedMixin):
    ...
    def add_transaction(self, transaction):
        # XXX thread safety on order - use a raw sql query instead?
        WorkBasketTransaction.objects.create(
            transaction=transaction,
            workbasket=self,
            order=self.transactions.count() + 1,
        )

    def new_transaction(self, **kwargs):
        """Create a new transaction in this workbasket."""

        transaction = self.transactions.model.objects.create(**kwargs)
...
class WorkBasketTransaction(models.Model):
    """A Transaction may be included in a single WorkBasket."""

    order = models.IntegerField()
    transaction = models.ForeignKey("common.Transaction", on_delete=models.CASCADE)
    workbasket = models.ForeignKey(WorkBasket, on_delete=models.CASCADE)
