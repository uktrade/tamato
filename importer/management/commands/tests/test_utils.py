...
from importer.management.commands.utils import NomenclatureTreeCollector, parse_trade_remedies_duty_expression, \
    convert_eur_to_gbp
...
def test_parse_duty_expression():
    ...
    expression = parse_trade_remedies_duty_expression('Cond: A cert: D-017 (01):0.000 % ; A cert: D-018 (01):28.200 % ; A (01):28.200 %')
...
def test_parse_duty_expression_with_conversion():
    expression = parse_trade_remedies_duty_expression('Cond: A cert: D-017 (01):10.000 EUR', eur_gbp_conversion_rate=2)

    assert expression[0].component.duty_amount == '20.000'
    assert expression[0].component.monetary_unit_code == 'GBP'

    expression = parse_trade_remedies_duty_expression('Cond: A cert: D-017 (01):10.000 USD', eur_gbp_conversion_rate=2)
