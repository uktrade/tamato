...
class TestImportTaricCommand:
    ...
    def test_help(self):
        ...
        assert 'name                  The name of the batch, the Envelope ID is recommended.' in out
        assert '-u USERNAME, --username USERNAME' in out
        assert 'The username to use for the owner of the workbaskets' in out
        assert '-S {ARCHIVED,EDITING,PROPOSED,APPROVED,SENT,PUBLISHED,ERRORED}, ' \
               '--status {ARCHIVED,EDITING,PROPOSED,APPROVED,SENT,PUBLISHED,ERRORED}' in out
