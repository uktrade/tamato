...
def test_footnote_type_xml(api_client, taric_schema, xml):
...
def test_footnote_xml(api_client, taric_schema, xml):
    element = xml.find(".//footnote", xml.nsmap)
    assert element is not None


@validate_taric_xml(factories.FootnoteDescriptionFactory)
def test_footnote_description_xml(api_client, taric_schema, xml):
