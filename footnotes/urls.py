...
footnote_detail = (
    r"^footnotes/(?P<footnote_type_id>[A-Z]{2,3})(?P<footnote_id>[0-9]{3}|[0-9]{5})/"
)
...
urlpatterns = [
    ...
    re_path(
        (footnote_detail + r"description/(?P<period_sid>[0-9]{1,8})/edit/$"),
        views.FootnoteDescriptionUpdate.as_view(),
        name="footnote-ui-description-edit",
    ),
    re_path(
        (footnote_detail + r"description/(?P<period_sid>[0-9]{1,8})/confirm-update/$"),
