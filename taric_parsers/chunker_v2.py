...
# Should this live on the ImportXmlChunk object? seems appropriate
...
def create_chunk(
    ...
    """
    Parses a TARIC3 XML stream and create an ImporterXmlChunk.

    All chunks are written to the database.

    Returns:
         ImporterXmlChunk or None, the created chunk or nothing if there was an issue.
    """

    # Check import size and error if too large

    # set file position to start of stream
    taric3_file.seek(0, os.SEEK_SET)

    bs4.BeautifulSoup(taric3_file, "xml")

    # # collect transactions
    # if record_group:
    #     transactions = taric_file_bs.find_all(lambda tag: tag.name == "loc")
