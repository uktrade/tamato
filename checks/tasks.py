...
def get_checker_model_rules(
    models: Sequence[TrackedModel],
    rule_names: Optional[Set[str]] = None,
):
    """
    Generator of model, rules.

    Given a sequence of models and a sequence of checkers
    ...
    for model in models:
        for checker in ALL_CHECKERS.values():
            yield from (
                (checker, checker_model, checker_rules)
                for checker_model, checker_rules in checker.get_model_rules(
                    model,
                    rule_names,
                ).items()
            )
...
def check_model(
    ...
    for checker in ALL_CHECKERS.values():
        for checker_model, rules in checker.get_model_rules(model, rule_names).items():
